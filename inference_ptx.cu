...
// 一些 PTX 辅助函数，用于显式使用 PTX 指令（ld.global.ca / fma.rn）
__device__ __forceinline__ float ptx_ld_ca_f32(const float* addr) {
    float val;
    asm volatile("ld.global.ca.f32 %0, [%1];" : "=f"(val) : "l"(addr));
    return val;
}

__device__ __forceinline__ unsigned char ptx_ld_ca_u8(const unsigned char* addr) {
    unsigned int tmp;
    asm volatile("ld.global.ca.u8 %0, [%1];" : "=r"(tmp) : "l"(addr));
    return static_cast<unsigned char>(tmp);
}

__device__ __forceinline__ float ptx_fma_rn_f32(float a, float b, float c) {
    float res;
    asm volatile("fma.rn.f32 %0, %1, %2, %3;" : "=f"(res) : "f"(a), "f"(b), "f"(c));
    return res;
}
            ...
            if (kernel_size == 5) {
                ...
                for (int kh = 0; kh < 5; kh++) {
                    ...
                    for (int kw = 0; kw < 5; kw++) {
                        ...
                        float a = ptx_ld_ca_f32(&input[input_idx]);
                        float w = ptx_ld_ca_f32(&weight[weight_idx]);
                        sum = ptx_fma_rn_f32(a, w, sum);
                    }
                }
            } else {
                #pragma unroll 2
    ...
    if (idx < total_output) {
        ...
        if ((in_features & 3) == 0) {
            ...
            for (int i = 0; i < iters; ++i) {
                float4 iv = in4[i];
                float4 wv = w4[i];
                sum = ptx_fma_rn_f32(iv.x, wv.x, sum);
                sum = ptx_fma_rn_f32(iv.y, wv.y, sum);
                sum = ptx_fma_rn_f32(iv.z, wv.z, sum);
                sum = ptx_fma_rn_f32(iv.w, wv.w, sum);
            }
        } else {
            for (int i = 0; i < in_features; i++) {
                float a = __ldg(&in_ptr[i]);
                float w = __ldg(&w_ptr[i]);
                sum = ptx_fma_rn_f32(a, w, sum);
            }
        }
        output[idx] = sum;
...
__global__ void linear_if_tiled_smem_kernel(const float* __restrict__ input, float* __restrict__ output_spike,
    ...
    if ((in_features & 3) == 0) {
        ...
        for (int i = 0; i < in_features; ++i) {
            sum = ptx_fma_rn_f32(sm_in[i], __ldg(&w_ptr[i]), sum);
        }
    }
    ...
    )
{
    ...
    const int max_batch = std::min(num_images, 1024);
    ...
    int conv_bs = global_bs ? global_bs : parse_block("BLOCK_SIZE_CONV", 352);
    int fc_bs   = global_bs ? global_bs : parse_block("BLOCK_SIZE_FC",   224);
    ...
    for (int batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
        ...
        dim3 conv2_grid((current_batch * conv2_out_channels * pool2_out_h * pool2_out_w + block_size_conv.x - 1) / block_size_conv.x);
        ...
        if (graph_exec[cur] == nullptr || graph_batch_size[cur] != current_batch) {
            ...
            for (int t = 0; t < T; ++t) {
                ...
                // Conv2 使用 U8 稀疏输入
                conv2d_if_pool2x2_u8_kernel<<<conv2_grid, block_size_conv, 0, compute_stream>>>(
        ...
        checkCudaErrors(cudaMemcpyAsync(batch_final_output.data(), d_final_out[cur],
                                        current_batch * fc3_out_features * sizeof(float),
                ...
                if (kernel_size == 5) {
                    ...
                    for (int kh = 0; kh < 5; ++kh) {
                        ...
                        for (int kw = 0; kw < 5; ++kw) {
                            ...
                            float a = ptx_ld_ca_f32(&input[in_idx]);
                            float w = ptx_ld_ca_f32(&weight[w_idx]);
                            sum = ptx_fma_rn_f32(a, w, sum);
                        }
                    }
                } else {
            ...
            for (int ic = 0; ic < in_channels; ++ic) {
                ...
                for (int kh = 0; kh < 5; ++kh) {
                    ...
                    for (int kw = 0; kw < 5; ++kw) {
                        int iw = ow + kw;
                        int in_idx = input_c_base + ih * input_w + iw;
                        unsigned char s = ptx_ld_ca_u8(&input_u8[in_idx]);
                        if (s) {
                            int w_idx  = weight_c_base + kh * 5 + kw;
                            float w = ptx_ld_ca_f32(&weight[w_idx]);
                            sum = ptx_fma_rn_f32(static_cast<float>(s), w, sum);
                        }
                    }
                }
            }
